...
namespace objl
{
	...
	public:
		...
		{
			std::string objPath = "Objects/" + Path;
	...
	private:
		...
		{
			std::string mtlPath = "Materials/" + path;

			// If the file is not a material file return false
			if (mtlPath.substr(mtlPath.size() - 4, mtlPath.size()) != ".mtl")
				return false;
			...
			while (std::getline(file, curline))
			{
				...
				std::string texturePath = "Materials/Textures/";
				// Ambient Texture Map
				if (algorithm::firstToken(curline) == "map_Ka")
				{
					tempMaterial.map_Ka = texturePath + algorithm::tail(curline);
				}
				// Diffuse Texture Map
				if (algorithm::firstToken(curline) == "map_Kd")
				{
					tempMaterial.map_Kd = texturePath + algorithm::tail(curline);
				}
				// Specular Texture Map
				if (algorithm::firstToken(curline) == "map_Ks")
				{
					tempMaterial.map_Ks = texturePath + algorithm::tail(curline);
				}
				// Specular Hightlight Map
				if (algorithm::firstToken(curline) == "map_Ns")
				{
					tempMaterial.map_Ns = texturePath + algorithm::tail(curline);
				}
				// Alpha Texture Map
				if (algorithm::firstToken(curline) == "map_d")
				{
					tempMaterial.map_d = texturePath + algorithm::tail(curline);
				}
				// Bump Map
				if (algorithm::firstToken(curline) == "map_Bump" || algorithm::firstToken(curline) == "map_bump" || algorithm::firstToken(curline) == "bump")
				{
					tempMaterial.map_bump = texturePath + algorithm::tail(curline);
				}
			}
